...
function final_optimize_action(planner, s, actions, velocity_set)  
...
function test_final_optimize_action(planner)
    ...
    final_optimize_action(planner,state,actions,velocity_set)
end
#=
p = HJBPlanner(RG[:solver],RG[:problem],750.0)
@benchmark test_final_optimize_action($p)
@profview for i in 1:10000 test_final_optimize_action(p) end
=#
...
function final_HJB_policy(planner, state)
    ...
    if state[4] == 0.0
        ...
        best_value, best_action_index = final_optimize_action(planner,state,actions,velocity_set) 
